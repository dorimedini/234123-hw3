...
typedef struct thread_pool {
	...
	/**
	 * Regular mutex lock for accessing the pool's state (RESOURCE ORDER=1)
	 */
	State state; 					// Represents the structure's state. Can be ALIVE, DO_ALL, or DO_RUN.
	pthread_mutexattr_t mutex_type;	// Use this to specify the ERRORCHECK type of mutex (for both locks)
	pthread_mutex_t state_lock;		// Lock this to read/write to the state.
	
	/**
 	 * Monitor (RESOURCE ORDER=0)
	 *
